...
def run_pytorch_training(config, model_filename, training_log_filename, verbose=1, use_multiprocessing=False,
    ...
    tab_config = None
    if "tabnet_params" in config:
        tab_config = config["tabnet_params"]
        optimizer_cls_kwargs["learning_rate"] = tab_config["optimizer_params"]
    ...
    #cat_id,cat_dim = training_dataset.get_data_info()
    ...
    # scheduler_params=tab_config["scheduler_params"]
    # learning_rate_decay_step_size = scheduler_params["step_size"]
    # decay_factor = scheduler_params["gamma"]
    #scheduler_cls = torch.optim.lr_scheduler.StepLR(optimizer=optimizer_cls, step_size=learning_rate_decay_step_size,
    #                                            gamma=decay_factor, last_epoch=-1)

    # train(model=model, optimizer=optimizer,optimizer_cls=optimizer_cls, criterion=criterion, n_epochs=config["n_epochs"], verbose=bool(verbose),
    ...
    #       amp=amp,scheduler_cls = scheduler_cls)
    return validation_loader,model


def train(model, optimizer,optimizer_cls, criterion, n_epochs, training_loader, validation_loader, training_log_filename,
          model_filename, metric_to_monitor="val_cls", early_stopping_patience=None,
          ...
          save_last_n_models=None, amp=False,scheduler_cls=None):
    training_log = list()
    if os.path.exists(training_log_filename):
        ...
        start_epoch = 0
    training_log_header = ["epoch", "loss","cls_loss", "lr","lr_cls", "val_loss","val_cls", "val_acc"]
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        metric = my_metric()
        # train the model
        loss,cls_loss = epoch_training(training_loader, model, criterion,metric = metric, optimizer=optimizer,optimizer_cls=optimizer_cls, epoch=epoch, n_gpus=n_gpus,
        ...
        if validation_loader:
            val_loss,val_cls,val_acc = epoch_validatation(validation_loader, model, criterion,metric=metric, n_gpus=n_gpus, regularized=regularized,
        ...
        training_log.append([epoch, loss,cls_loss, get_lr(optimizer),get_lr(optimizer_cls), val_loss,val_cls,val_acc])
