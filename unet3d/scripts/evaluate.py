...
import warnings
from unet3d.utils.affine import resize_affine
...
def parse_args():
    ...
    input_group.add_argument("--directory",default='/home/xiao/project/3DUnetCNN/output_results/output_results_all_test/fold0',
    ...
    parser.add_argument("--config_filename", default='/home/xiao/project/3DUnetCNN/examples/all504/all_config_test.json')
    parser.add_argument("--output_filename", default='/home/xiao/project/3DUnetCNN/evaluate/all_test/fold0.csv',
...
def evaluate_filenames(filename1, filename2, labels):
    ...
    affine = resize_affine(image1.affine,image1.shape,window)
    image1 = resample(image=image1,target_affine=affine,target_shape=window,interpolation='nearest')
...
def main():
    ...
    for filename in filenames:
        subject_id = os.path.basename(filename)[:-7]
        subject_ids.append(subject_id)
        target_filenames.append(os.path.join(config["generate_filenames_kwargs"]["directory"],
                                             config["generate_filenames_kwargs"]["target_templates"].format(
                                                 subject=subject_id)))
