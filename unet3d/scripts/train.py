...
# def get_table_data():
#     #out = '/media/xiao/新加卷/3DUnetCNN_local/table_data/test_tabnet.csv'
#     #project_dir = os.path.dirname(sys.argv[0])
#     out = os.path.join(project_dir,'table_data/new_data_eng.csv')
...
#     #id_list = train['id_new']
#     # train_list = id_list[train_indices]
#     # json_f =  open('/media/xiao/新加卷/3DUnetCNN/examples/all504/train_list.json',mode='w')
#     # json.dump(train_list.to_list(),json_f)
#     # val_list = id_list[valid_indices]
#     # json_f = open('/media/xiao/新加卷/3DUnetCNN/examples/all504/val_list.json', mode='w')
#     # json.dump(val_list.to_list(), json_f)
#     # test_list = id_list[test_indices]
#     # json_f = open('/media/xiao/新加卷/3DUnetCNN/examples/all504/test_list.json', mode='w')
#     # json.dump(test_list.to_list(), json_f)
...
#             train.fillna(train.loc[valid_indices, col].mean(), inplace=True)
#             train.fillna(train.loc[test_indices, col].mean(), inplace=True)
#
#     unused_feat = ['NIHSS']
#     #unused_feat = ['NIHSS','location','shifts','smokes','drinks','diabetes','myocardials','coronarys','atrias','hypertensions','strokes'
#     #                'loc','shift','smoke','drink','diabete','myocardial','coronary','atria','hypertension','stroke']
#     #unused_feat = ['NIHSS','location','shifts','smokes','drinks','diabetes','myocardials','coronarys','atrias','hypertensions','strokes']
#
...
# import os
# os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:320"
def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config_filename",default=project_dir+'/examples/all504/all_config_tableunet.json',
    ...
    parser.add_argument("--model_filename",
                        ...
                        default=project_dir+'/models/model_train/all_unet3d.h5')
    parser.add_argument("--training_log_filename",
                        ...
                        default=project_dir+'/models/model_train/all_training_log.csv')
...
def main():
    ...
    if namespace.sub_command == "predict":
        return run_inference(namespace)
    return run_training(package, config, namespace.model_filename, namespace.training_log_filename,
                 sequence_class=sequence_class,
                 model_metrics=model_metrics,
                 metric_to_monitor=metric_to_monitor,
                 bias=bias,
