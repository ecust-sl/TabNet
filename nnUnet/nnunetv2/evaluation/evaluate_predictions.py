...
from score_gt import img_resample
import warnings
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    # aspect = sitk.ReadImage('/home/xiao/nnUNet/aspect_template_extract_regist.nii.gz')
    #
    # size = [193,229,96]
    # aspect = img_resample(aspect, size)
    # aspect_np = sitk.GetArrayFromImage(aspect)
    # #aspect_np = aspect_np.transpose(0,2,1)
    # aspect_np = np.expand_dims(aspect_np,axis=0)
    ######################
    ...
    for r in labels_or_regions:
        ...
        # results_per_area = []
        # for i in range(10):
        #     area_mask = np.zeros_like(mask_pred)
        #     area_mask[aspect_np == 2 * (i + 1)] = 1
        #     area_mask[aspect_np == i + 1] = 1
        #     mask_ref_i = mask_ref * area_mask
        #     mask_pred_i = mask_pred * area_mask
        #     tp_i, fp_i, fn_i, tn_i = compute_tp_fp_fn_tn(mask_ref_i, mask_pred_i, ignore_mask)
        #     results_per_area.append(2 * tp_i / (2 * tp_i + fp_i + fn_i+1e-6))
            ###############################
        if tp + fp + fn == 0:
    ...
    # data为list类型
    # row = ''
    # for i in results_per_area:
    #     row+=str(i)+','
    # print(reference_file.split('/')[-1].split('.')[0]+','+row)
    return results
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    """
    # folder_ref_1 = '/media/xiao/DATA/dataset/nnUNet_raw/Dataset507_All/labelsTs'
    # folder_pred_1 = '/media/xiao/DATA/dataset/nnUNet_raw/Dataset507_All/predTs'
    if output_file is not None:
        assert output_file.endswith('.json'), 'output_file should end with .json'
    files_pred = subfiles(folder_pred, suffix=file_ending, join=False)
    files_pres_1 = subfiles(folder_pred_1, suffix=file_ending, join=False)
    files_ref = subfiles(folder_ref, suffix=file_ending, join=False)
    files_ref_1 = subfiles(folder_ref_1, suffix=file_ending, join=False)
    if not chill:
        present = [isfile(join(folder_pred, i)) for i in files_ref]
        assert all(present), "Not all files in folder_pred exist in folder_ref"
    files_ref = [join(folder_ref, i) for i in files_pred] + [join(folder_ref_1, i) for i in files_ref_1]
    files_pred = [join(folder_pred, i) for i in files_pred] + [join(folder_pred_1, i) for i in files_pres_1]
    ...
    # import csv
    # path = './area_dice_test.csv'
    # # path为输出路径和文件名，newline=''是为了不出现空行
    # csvFile = open(path, "w+", newline='')
    # writer = csv.writer(csvFile)
    # for i in results_area:
    #     writer.writerow(i)
...
if __name__ == '__main__':
    folder_ref = '/media/xiao/DATA/dataset/nnUNet_raw/Dataset507_All/labelsOutTs'
    folder_pred = '/media/xiao/DATA/dataset/nnUNet_raw/Dataset507_All/predOutTs'

    # folder_ref = '/home/xiao/nnUNet/nnUNetFrame/DATASET_train/nnUNet_raw/nnUNet_raw_data/Task504_All/labelsTr'
    # folder_pred = '/home/xiao/nnUNet/nnUNetFrame/DATASET_train/nnUNet_raw//nnUNet_raw_data/Task504_All/predTr'
    #output_file = '/home/xiao/nnUNet/nnUNetFrame/DATASET/nnUNet_raw/Dataset_AllTest/summary.json'
    output_file = '/home/xiao/nnUNet/new_score/score_03.01_xh/model_xinhua_avg.json'
