...
def convert_msd_dataset(source_folder: str, overwrite_target_id: Optional[int] = None,
    ...
    imagesOutTs = join(source_folder, 'imagesOutTs')
    labelsOutTs = join(source_folder, 'labelsOutTs')
    ...
    target_imagesOutTs = join(target_folder, 'imagesOutTs')
    target_labelsTr = join(target_folder, 'labelsTr')
    target_labelsTs = join(target_folder, 'labelsTs')
    target_labelsOutTs = join(target_folder, 'labelsOutTs')
    maybe_mkdir_p(target_imagesTr)
    maybe_mkdir_p(target_imagesTs)
    maybe_mkdir_p(target_imagesOutTs)
    maybe_mkdir_p(target_labelsTr)
    maybe_mkdir_p(target_labelsTs)
    maybe_mkdir_p(target_labelsOutTs)
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as p:
        ...
        source_images = [i for i in subfiles(imagesOutTs, suffix='.nii.gz', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
        source_images = [join(imagesOutTs, i) for i in source_images]

        results.append(
            p.starmap_async(
                split_4d_nifti, zip(source_images, [target_imagesOutTs] * len(source_images))
            )
        )
        ...
        source_images = [i for i in subfiles(labelsOutTs, suffix='.nii.gz', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
        for s in source_images:
            shutil.copy(join(labelsOutTs, s), join(target_labelsOutTs, s))
        [i.get() for i in results]
...
if __name__ == '__main__':
    convert_msd_dataset('/home/xiao/nnUNet/nnUNetFrame/DATASET/nnUNet_raw/Task505_All/')
