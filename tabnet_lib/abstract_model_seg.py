...
class TabModel(BaseEstimator):
    ...
    def fit(
        ...
        # else:
            #weight = torch.tensor([1/0.46,1/0.33,1/0.21]).to(self.device)
        ...
        if not hasattr(self, "network") or not warm_start:
            ...
            self._set_network(model_seg)
        self._update_network_params()
        self._set_metrics(eval_metric, eval_names)
        self._set_optimizer()
        self._set_callbacks(callbacks)

        if tr_loader is not None and val_loader is not None:
            train_dataloader = tr_loader
            valid_dataloaders = [tr_loader, val_loader]
        ...
        for epoch_idx in range(self.max_epochs):
            ...
            for eval_name, valid_dataloader in zip(eval_names, valid_dataloaders):
                best_score = self._predict_epoch(eval_name, best_score,valid_dataloader,epoch_idx)
    ...
    def save_model(self, path,best_score):
        ...
        print(f"Best valid custom : {best_score}")
        return f"{path}.zip"
    ...
    def _train_epoch(self, train_loader):
        ...
        for batch_idx, (img,label,X, y) in enumerate(train_loader):
            self._callback_container.on_batch_begin(batch_idx)
            #batch_logs = None
            batch_logs = self._train_batch(img,label,X, y)
    ...
    def _train_batch(self, img,label,X, y):
        """
        ...
        msk = label.to(self.device).float()
        ...
        seg_out, output, M_loss = self.network(img,X)


        # Add the overall sparsity loss
        loss_cls = self.compute_loss(output, y) - self.lambda_sparse * M_loss
        #loss_cls.backward(retain_graph=True)
        loss_seg = self.loss_seg(seg_out, msk)
        #loss_seg.backward()
        ...
        self._optimizer_cls.step()

        #batch_logs["loss"] = loss_seg.cpu().detach().numpy().item()
        batch_logs["loss_cls"] = loss_cls.cpu().detach().numpy().item()

        return batch_logs

    def _predict_epoch(self, name,best_auc, loader,epoch_idx):
        """
        ...
        list_y_true = []
        list_y_score = []
        self.device_val = torch.device(f'cuda:2')
        # Main loop

        cur_auc = 0
        for batch_idx, (img,label,X, y) in enumerate(loader):
            scores,_ = self._predict_batch(img,X)
            list_y_true.append(y)
            list_y_score.append(scores)
        y_true, scores = self.stack_batches(list_y_true, list_y_score)
        metrics_logs = self._metric_container_dict[name](y_true, scores)
        # if name == 'val':
        #     cur_auc = metrics_logs['val_custom']
        #     if cur_auc > best_auc:
        #         best_auc = cur_auc
        #         self.save_model('../models/tabunet',best_auc)
        self.network.train()
        self.history.epoch_metrics.update(metrics_logs)
        return best_auc

    def _predict_batch(self, img,X):
        """
        ...
        seg_out, scores,_ = self.network(img,X)
        ...
        return scores,seg_out

    def _set_network(self,model_seg):
    ...
    def _set_callbacks(self, custom_callbacks):
        ...
        if self.scheduler_fn is not None:
            ...
            scheduler_cls = LRSchedulerCallback(
                scheduler_fn=self.scheduler_fn,
                scheduler_params=self.scheduler_params,
            ...
            callbacks.append(scheduler_cls)
    ...
    def _set_optimizer(self):
        ...
        for name,param in self.network.named_parameters():
            if name.split('.')[0] not in seg_list:
                cls_list.append(param)
        """Setup optimizer."""
        self._optimizer = self.optimizer_fn(
            cls_list, **self.optimizer_params
        )
        self._optimizer_cls = self.optimizer_fn(
            self.network.model_seg.parameters(), lr=1e-4
        )
