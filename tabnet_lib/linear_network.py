...
class LinearNet(torch.nn.Module):
    def __init__(
        ...
        feture_size0 = 4*4*4*512
        feture_size1 = 8*8*8*256
        feture_size2 = 16*16*16*128


        # self.final_mapping_2 = torch.nn.Linear(6, 3, bias=False)
        # initialize_non_glu(self.final_mapping_2, 6, 3)

        self.final_mapping1 = torch.nn.Linear(self.output_dim, self.output_dim, bias=False)
        initialize_non_glu(self.final_mapping1, self.output_dim, self.output_dim)
        self.final_mapping2 = torch.nn.Linear(36, 32, bias=False)
        initialize_non_glu(self.final_mapping2, 36, 32)
        self.final_mapping3 = torch.nn.Linear(32, self.output_dim, bias=False)
        initialize_non_glu(self.final_mapping3, 32, self.output_dim)
        #self.dp = torch.nn.Dropout(0.8)

    def forward(self, x):
        # emb_x = self.embedder(x)
        ...
        #fl_seg_x = self.conv_reduce(fl_seg_x)
        ...
        cls_x = self.final_mapping1(x)
        return img, cls_x, m_loss

        # cls_x = torch.einsum("bk,bk->bk",att_1*fl_seg_x,att_2*x)
        # cls_x = self.conv_ein2(cls_x)
        # return img, cls_x, m_loss
